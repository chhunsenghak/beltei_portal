...
const _kAdmin = (
  ...
  email: 'sophal.nget@beltei.edu.kh',
...
class _AdminProfileScreenState extends State<AdminProfileScreen> {
  ...
  void initState() {
    ...
    for (final group in _kSystemSettings) {
      for (final item in group.items) {
        if (item.isToggle) {
          _toggles[item.label] = item.toggleValue;
        }
      }
    }
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(_kAdmin.name, style: AppTextStyles.h2White),
          const SizedBox(height: 4),
          Text(_kAdmin.role,
              style:
                  AppTextStyles.captionWhite.copyWith(fontSize: 13)),
  ...
  Widget _buildAdminInfo() {
    final details = [
      (icon: Icons.email_outlined,    label: 'Email',      value: _kAdmin.email),
      (icon: Icons.phone_outlined,    label: 'Phone',      value: _kAdmin.phone),
      (icon: Icons.access_time_outlined, label: 'Last Login', value: _kAdmin.lastLogin),
    ];
    return Container(
